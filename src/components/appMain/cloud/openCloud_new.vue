			...
			<div v-if="!vmsLoading" class="cloud__container">
				<div class="Fcloud">
					<div class="Fcloud__header">
						...
						<div class="Fcloud__header-title" v-if="VM && VM.status">
							<div class="Fcloud__status-color" :class="{ 'glowing-animations': updating }" :style="{'background-color': stateColor}"></div>
					...
					<div class="Fcloud__buttons">
						...
						<div class="Fcloud__button" :class="{ 'btn_disabled_wiggle': true}" @click='sendAction("reboot")'>
							<div class="Fcloud__BTN-icon">
								<a-icon type="redo" />
							</div>
							<div class="Fcloud__BTN-title">
							...
							<a-modal v-model="modal.reboot" :title="$t('cloud_Reboot_modal')" @ok="sendAction('suspend')">
						...
						<div class="Fcloud__button" :class="{ 'btn_disabled_wiggle': true}" @click='openModal("recover")'>
							<div class="Fcloud__BTN-icon">
						...
						<div class="Fcloud__info-block block">
							...
							<div class="Fcloud__block-content">
								<div class="block__column">
									...
									<div class="block__value">{{(VM && VM.os) || 'no data'}}</div>
								</div>
								<div class="block__column">
						...
						<div class="Fcloud__info-block block">
							...
							<div class="Fcloud__block-content">
								<div class="block__column">
									...
									<div class="block__value">{{VM.resources.drive_type}}</div>
								</div>
								<div class="block__column">
									<div class="block__title">{{$t('cloud_Size')}}</div>
									<div class="block__value">{{mbToGb(VM.resources.drive_size)}} GB</div>
								</div>
							</div>
						</div>
...
export default {
	...
	methods: {
		...
		sendAction(action){
			let vminfo = {
				service: this.VM.meta.serviceUUID,
				group: this.VM.meta.groupName,
				instance: this.VM.uuid,
			}
			this.actualAction = action;
			this.actionLoading = true;
			api.services.action(vminfo, action)
			.then(() => {
				const opts = {message: `Done!`}
				console.log(opts)
			})
			.catch(err => {
				const opts = {
